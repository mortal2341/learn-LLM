...
# 加载预训练数据(300万中文指令微调数据)
raw_datasets = datasets.load_dataset(
    "json", data_files="data/processed_chinese_instruct_300w.jsonl"
)
...
# 保存测试集用于后续评估
raw_datasets["test"].to_json(path_or_buf = './data/sft_plus_train_test_30k.json', force_ascii=False)
...
args = transformers.TrainingArguments(
    output_dir="saved/full_sft_plus",
...
# 保存模型和tokenizer
model.save_pretrained("./saved/full_sft/sft-plus-qwen-0.1B/")  # 保存模型的路径
tokenizer.save_pretrained("./saved/full_sft/sft-plus-qwen-0.1B/")


# 保存训练日志
with open("log/full_sft_plus_running_states.json", "w", encoding="utf-8") as fw:
