...
raw_datasets = datasets.load_dataset(
    "json", data_files="data/processed_baike_sft_120w.jsonl"
)
...
# 保存测试集用于后续评估
raw_datasets["test"].to_json(path_or_buf = './data/sft_baike_test_12k.json', force_ascii=False)
...
# 保存模型和tokenizer
model.save_pretrained("./saved/full_sft/sft-baike-qwen-0.1B/")  # 保存模型的路径
tokenizer.save_pretrained("./saved/full_sft/sft-baike-qwen-0.1B/")


# 保存训练日志
with open("log/full_sft_running_states.json", "w", encoding="utf-8") as fw:
