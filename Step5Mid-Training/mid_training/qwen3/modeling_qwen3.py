...
class Qwen3RotaryEmbedding(nn.Module):
    def __init__(self, config: Qwen3Config, device=None):
        ...
        if self.rope_type == "yarn":
            import math
            rope_scaling, rope_base, dim = config.rope_scaling, config.rope_theta, config.head_dim
            ...
            if config.max_position_embeddings / orig_max > 1.0:
                # YaRN formula: f_new(i) = f_old(i)((1-γ) + γ/s)
                inv_dim = lambda x: (dim * math.log(orig_max / (x * 2 * math.pi))) / (2 * math.log(rope_base))
                low, high = max(math.floor(inv_dim(beta_fast)), 0), min(math.ceil(inv_dim(beta_slow)), dim // 2 - 1)
                ramp = torch.clamp((torch.arange(dim // 2, device=inv_freq.device).float() - low) / max(high - low, 0.001), 0, 1)
            ...
            if self.rope_type in ROPE_INIT_FUNCTIONS:
                ...
                # transformers >=5.0 removed the "default" entry from ROPE_INIT_FUNCTIONS.
                # Compute the vanilla RoPE inverse frequencies directly.
