...
sp_en.Load('jieduan 0\RNN translator\en_bpe.model')
sp_cn = spm.SentencePieceProcessor()
sp_cn.Load('jieduan 0\RNN translator\zh_bpe.model')

#encode解码后是纯数字文本，机器无法处理中文和英文
...
class TranslationDataset(Dataset):
    ...
    def __init__(self, src_file, trg_file, src_tokenizer, trg_tokenizer, max_len=100):
        ...
        for src, trg in zip(src_lines, trg_lines):
            # 每个句子前边增加<bos>后边增加<eos>，通过分词使其成为纯数字
    ...
    # 将init中的List列表转换成longtensor长整形的张量,！！！embedding嵌入层输入必须是长整形的张亮
    def __getitem__(self, idx):
        src_ids, trg_ids = self.pairs[idx]
        return torch.LongTensor(src_ids), torch.LongTensor(trg_ids)
    #这段代码定义了一个 collate_fn 函数，它是 PyTorch DataLoader 的核心组件，
    # 专门用于处理变长序列的批处理（batching）。在机器翻译等 NLP 任务中，句子长度各不相同，
#    这个函数负责将多个变长序列动态填充（padding） 成统一形状的张量，确保 GPU 能高效并行计算。
    @staticmethod
...
if __name__ == '__main__':
    ...
    dataset = TranslationDataset('C:\\Users\\1\\Desktop\\learn-torch\\data\\en2cn\\train_en.txt', 
                                 'C:\\Users\\1\\Desktop\\learn-torch\\data\\en2cn\\train_zh.txt', tokenize_en, tokenize_cn)
    ...
    if os.path.exists('seq2seq_bpe_attention.pt'):
