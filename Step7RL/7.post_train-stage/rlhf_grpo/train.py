...
def get_batch():
    ...
    data['ref_logps'] = bytes_to_tensor(dd[3])
...
def GRPO_step(batch):
    ...
    logits = engine(inputs).logits        # 计算策略模型的logp，(batch, seq_len, vocab)
    ...
    ref_per_token_logps = batch['ref_logps'].to(per_token_logps.device) # (batch, answer_len)
    ...
    # log(p) - log(old_p)
    ratio = torch.exp(per_token_logps - old_per_token_lops)
    
    # grpo clip ratio
    clipped_ratio = torch.clamp(ratio, 1-config["training"]["clip_param"], 1+config["training"]["clip_param"])
    
    # min(ratio*A, clip_ratio*A)
    per_token_loss = torch.min(ratio * advantages, clipped_ratio * advantages)
    
    # 加上KL约束
    per_token_loss = -(per_token_loss - config["training"]["beta"] * per_token_kl)
...
def DAPO_step(batch):
    ...
    # grpo clip ratio (Clip Higher)
    clipped_ratio = torch.clamp(ratio, 1-config["training"]["clip_param"], 1+config["training"]["clip_param_higher"])
    
    # min(ratio*A, clip_ratio*A)
    per_token_loss = torch.min(ratio * advantages, clipped_ratio * advantages)
    
    # DAPO 去掉了KL约束
    per_token_loss = -per_token_loss # (batch, answer_len)
    
    # 取全局的token平均 (Token-Level Policy Gradient Loss)
...
def GSPO_step(batch):
    ...
    # 计算有效序列，遮掩的pad_token
    valid_seq_len = completion_mask.sum(dim=1) # (batch, )
    batch_size = per_token_logps.shape[0]

    # log(p) - log(old_p)
    new_old_log_probs = (per_token_logps - old_per_token_lops) * completion_mask # (batch, answer_len)

    # log(ref_p) - log(p) : k3估计里面的r
    ref_new_log_probs = (ref_per_token_logps - per_token_logps) * completion_mask # (batch, answer_len)

    # 序列级别的重要性采样
    # s_i(theta) = exp(mean(log(p) - log(old_p)))
    seq_ratio = torch.exp(new_old_log_probs.sum(dim=1) / valid_seq_len).view(batch_size, 1)  # (batch, 1)
    clipped_seq_ratio = torch.clip(seq_ratio, 1-config["training"]["clip_param"], 1+config["training"]["clip_param"])

    # K3估计: (r-1-log(r))
    kl_term = torch.exp(ref_new_log_probs.sum(dim=1) / valid_seq_len) - (ref_new_log_probs.sum(dim=1) / valid_seq_len) - 1  
    kl_term = kl_term.view(batch_size, 1)  # (batch, 1)

    # 计算loss
    loss = -(torch.min(seq_ratio * advantages, clipped_seq_ratio * advantages) - config["training"]["beta"] * kl_term).mean()
...
def gen_worker(model_queue, physics_device):
    ...
    ref_server_ver = 'tensor'  # 标识位, 用于请求参考模型的server，string和tensor两个标志来轮换
    Q_batch_size = config["training"]["Q_batch_size"]   # 一次rollout的prompt数
    num_pre_Q = config["training"]["num_pre_Q"]         # 每一条prompt采样的答案数
    train_batch_size = config["training"]["train_batch_size"]  # grpo/gspo训练的batch size
    ...
    def gen_samples(inputs, test=False):
        ...
        if test:
            ...
            for i, inp in enumerate(inputs):
                for answer, ans_token_id in zip(answers[i*num_pre_Q: (i+1)*num_pre_Q], 
                                                ans_token_ids[i*num_pre_Q: (i+1)*num_pre_Q]):
                    correct_score = reward_correct(inp, answer)
                    format_score = reward_format(inp, answer)
                    if config["training"]["use_dapo"]:
                        length_score = reward_length(ans_token_id, config["training"]["l_max"], config["training"]["l_cache"])
                        reward = correct_score + format_score + length_score
                    else:
                        reward = correct_score + format_score
                    rewards.append(reward)
                    rewards_info.append({
        ...
        for i, prompt in enumerate(prompt_inputs):
            ...
            if ref_server_ver == 'tensor':
                ...
                for ii in range(0, num_pre_Q, train_batch_size):      # minibatch_size, 训练批量大小
                    ...
                    Qrep = prompt_ids.repeat(1, output_ids.shape[0]).view(-1, prompt_len) # 同一条prompt复制train_batch_size份
                    merged_ids = torch.cat([Qrep, output_ids], dim=1)        #把prompt ids和answer ids拼接到一起
                    ...
                    # 把json转换成bytes (生产做法用二进制，快速原型开发可以用json，base64)
...
if __name__ == '__main__':
    ...
    engine, _, _, _ = deepspeed.initialize(config=ds_config, model=model, 
    ...
    for step in progress:
        ...
        elif config["training"]["use_gspo"]:
            loss = GSPO_step(batch)
        else:
